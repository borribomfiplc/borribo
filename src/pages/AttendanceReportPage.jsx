...
export default function AttendanceReportPage({ historyData = [] }) {
  ...
  return (
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-[#EBEDF3] bg-white">
        <div className="border-b border-[#EBEDF3] px-4 py-4 sm:px-5"><h3 className="text-[15px] font-semibold text-[#1E2333]">សង្ខេបតាមសាខា</h3></div>
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[720px] text-sm"><thead><tr className="bg-[#F7F8FB] text-xs text-[#8A8FA3]"><th className="px-5 py-3 font-medium">សាខា</th><th className="px-5 py-3 font-medium">មានវត្តមាន</th><th className="px-5 py-3 font-medium">យឺត</th><th className="px-5 py-3 font-medium">អវត្តមាន</th><th className="px-5 py-3 font-medium">ច្បាប់</th><th className="px-5 py-3 font-medium">អត្រាវត្តមាន</th></tr></thead><tbody>{Object.entries(branchStats).map(([branch, stats]) => <tr key={branch} className="border-t border-[#EBEDF3]"><td className="px-5 py-3.5 font-medium text-[#1E2333]">{branch}</td><td className="px-5 py-3.5 text-[#5B5F73]">{stats.present}</td><td className="px-5 py-3.5 text-[#5B5F73]">{stats.late}</td><td className="px-5 py-3.5 text-[#5B5F73]">{stats.absent}</td><td className="px-5 py-3.5 text-[#5B5F73]">{stats.leave}</td><td className="px-5 py-3.5 font-medium text-[#1E2333]">{stats.total ? (((stats.present + stats.late) / stats.total) * 100).toFixed(0) : 0}%</td></tr>)}{!Object.keys(branchStats).length && <tr><td colSpan={6} className="py-10 text-center text-[#8A8FA3]">មិនមានទិន្នន័យក្នុងរយៈពេលនេះទេ</td></tr>}</tbody></table></div>
        <div className="divide-y divide-[#EBEDF3] md:hidden">{Object.entries(branchStats).map(([branch, stats]) => { const rate = stats.total ? (((stats.present + stats.late) / stats.total) * 100).toFixed(0) : 0; return <article key={branch} className="p-4"><div className="flex items-center justify-between gap-3"><div className="font-semibold text-[#1E2333]">{branch}</div><span className="rounded-full bg-[#EEF1FB] px-2.5 py-1 text-xs font-semibold text-[#2A3F8F]">{rate}%</span></div><div className="mt-3 grid grid-cols-4 gap-2 text-center">{[["មានវត្តមាន",stats.present,COLORS.green],["យឺត",stats.late,COLORS.accent],["អវត្តមាន",stats.absent,COLORS.red],["ច្បាប់",stats.leave,COLORS.purple]].map(([label,value,color]) => <div key={label} className="rounded-xl bg-[#F7F8FB] px-1 py-2.5"><div className="text-base font-bold" style={{color}}>{value}</div><div className="mt-0.5 text-[10px] leading-tight text-[#8A8FA3]">{label}</div></div>)}</div></article>; })}{!Object.keys(branchStats).length && <div className="py-10 text-center text-sm text-[#8A8FA3]">មិនមានទិន្នន័យក្នុងរយៈពេលនេះទេ</div>}</div>
      </div>
    </>
  );
}
